...
# アザラシのクラスラベルとクラス名の対応関係（任意のクラス名を設定してください）
class_labels = {
    0: "猫",
    1: "犬",
    2: "アザラシ"
...
# 画像分類モデルの重みファイルのパス（任意のファイル名を設定してください）
model_weights_file = "keras_model.h5"
...
def main():
    ...
    if uploaded_file is not None:
        ...
        prediction = teachable_machine_classification(image, model_weights_file)
        ...
        probabilities = [round(p * 100, 2) for p in prediction]

        # 推論結果の表示
        st.subheader(f"クラス：{class_labels[classNo]}")
        st.subheader("推論結果")
